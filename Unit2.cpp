...
void __fastcall TForm2::Button1Click(TObject *Sender)
{
    ...
    if(lbl)
        MessageDlg("Заповніть обов'язкові поля!", mtInformation, TMsgDlgButtons() << mbOK, 0);
    else {
        ...
        if(ComboBox2->Text.Length() > 0 && Edit6->Text.Length() > 0 && Edit7->Text.Length() > 0)
            Memo1->Lines->Add("вул. " + ComboBox2->Text + ", буд. " + Edit6->Text + ", кв. " + Edit7->Text);
        else
            Memo1->Lines->Add("-");
        ...
        if(CheckBox1->Checked)
            arr1 += (CheckBox2->Checked || CheckBox3->Checked) ? "інвалідність, " : "інвалідність";
        if(CheckBox2->Checked)
            arr1 += (CheckBox3->Checked) ? "хронічні захворювання, " : "хронічні захворювання";
        if(CheckBox3->Checked)
            arr1 += (Edit9->Text.Length() > 0) ? Edit9->Text : (AnsiString)"інші";

        if(CheckBox4->Checked)
            arr2 += (CheckBox5->Checked || CheckBox6->Checked) ? "рентген, " : "рентген";
        if(CheckBox5->Checked)
            arr2 += (CheckBox6->Checked) ? "щеплення, " : "щеплення";
        if(CheckBox6->Checked)
            arr2 += "додаткові обстеження";
...
void __fastcall TForm2::Edit1KeyPress(TObject *Sender, char &Key)
{
    ...
    if(Sender == Edit2 || Sender == Edit3 || Sender == Edit4)
        if(Key != VK_BACK && (Key < 'а' || Key > 'я') && (Key < 'А' || Key > 'Я') && Key != 'і' && Key != 'І')
            Key = 0;
    if(Sender == Edit5)
...
void __fastcall TForm2::Button3Click(TObject *Sender)
{
    if(fmem) {
        ...
        MessageDlg("Спочатку сформуйте запис!", mtInformation, TMsgDlgButtons() << mbOK, 0);
}
//---------------------------------------------------------------------------
